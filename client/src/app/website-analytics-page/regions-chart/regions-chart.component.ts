...
@Component({
  selector: 'app-regions-chart',
  templateUrl: './regions-chart.component.html',
  styleUrls: ['./regions-chart.component.css']
})
export class RegionsChartComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    this.aSub$ = this.analyticsService.getWebsiteData(this.params).subscribe(
      (data: AnalyticsWebsiteData) => {
        ...
        for (let i = 0; i < data.chart.length; i++) {
          ...
          for (let j = 0; j < data.chart[i].regions.length; j++) {
            if (y.indexOf(data.chart[i].regions[j].name) !== -1) {
              continue;
            }
            y.push(data.chart[i].regions[j].name);

            obj[data.chart[i].regions[j].name] = {}
            obj2[data.chart[i].regions[j].name] = {}
            dates.forEach(el => obj2[data.chart[i].regions[j].name][el] = null)
          }
          for (let j = 0; j < data.chart[i].regions.length; j++) {
            obj[data.chart[i].regions[j].name][data.chart[i].label] = data.chart[i].regions[j].number;
          }
        }
