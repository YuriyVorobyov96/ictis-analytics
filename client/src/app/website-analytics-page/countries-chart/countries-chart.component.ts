...
export class CountriesChartComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    this.aSub$ = this.analyticsService.getWebsiteData(this.params).subscribe(
      (data: AnalyticsWebsiteData) => {
        ...
        for (let i = 0; i < data.chart.length; i++) {
          ...
          for (let j = 0; j < data.chart[i].countries.length; j++) {
            if (y.indexOf(data.chart[i].countries[j].name) !== -1) {
              continue;
            }
            y.push(data.chart[i].countries[j].name);

            obj[data.chart[i].countries[j].name] = {}
            obj2[data.chart[i].countries[j].name] = {}
            dates.forEach(el => obj2[data.chart[i].countries[j].name][el] = null)
          }
          for (let j = 0; j < data.chart[i].countries.length; j++) {
            obj[data.chart[i].countries[j].name][data.chart[i].label] = data.chart[i].countries[j].number;
          }
        }
